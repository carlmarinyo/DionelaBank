...
public class Library {
    ...
    public void saveItemsToFile() { // test
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("libraryItems.txt"))) {
            ...
            writer.close();
            System.out.println("Borrowemd item successfully..\n");
        } catch (IOException e) {
            ...
            for (Library items : availableItems) {
                if (items.getItemId() == itemId) { // PUT A CONDITION NA KAPAG MERON PANG QUANTITY OR WALA
                    ...
                    if (items.getQuantity() >= quantity && quantity > 0) {
                        ...
                        borrowItemInfo = new BorrowedItems(customerN, itemId, quantity); // tawag si borroweditem class
                                                                                         // Borrowitem, pra mailagay si
                                                                                         // Customer name at hiniram,
                                                                                         // nag popointer null exception
                                                                                         // pagnawala tu
                        borrowItemInfo.Borrowitems(customerN, itemId, quantity); // method sa borroweditems, di ko
                                                                                 // mafigure out sa file handling ksi eh
                        borrowInfoListCopy.add(borrowItemInfo); // add si borrowItemInfo sa arraylist ng borrowed items
                                                                // na borrowinfolistcopy
                ...
                if (borrowInfo.getItemId() == itemId) {
                    ...
                    if (quantity > borrowInfo.getQuantity()) {
                        ...
                        if (borrowInfo.getQuantity() == 0) {
                            ...
                            borrowInfoListCopy.remove(borrowInfo); // mareremove sa borrowers file yung nag borrow pag
